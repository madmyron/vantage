...
async function daxSend() {
  ...
  try {
    const res = await daxCall({
      model:      'claude-sonnet-4-20250514',
      max_tokens: 1000,
      system:     DAX_SYSTEM,
      messages:   daxHistory,
    });
    if (!res) { daxRemoveTyping(); daxTyping = false; return; }
...
async function daxSyncToConvo() {
  ...
  try {
    ...
    if (!res) { if (btn) { btn.textContent = 'Save failed - try again'; btn.disabled = false; } return; }
    const data = await res.json();
