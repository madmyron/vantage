...
| Messages disappear when second question sent | `sendMessage` captures `messages` state at call time; `messagesForApi` in reminder/list/default branches uses stale `messages` instead of `updated` | Use `[...updated.slice(0, -1), createMessage('user', enrichedText)]` — `updated` is `[...messages, userMsg]` built at the start of each call, so it's always current |
