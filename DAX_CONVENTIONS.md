...
| ESPN next-game returns nothing | `teams/{abbr}/schedule` only returns past games, never upcoming ones | Use `espnFindNextGame(sport, league, abbr)` helper that walks scoreboard by date (`/scoreboard?dates=YYYYMMDD`) up to 14 days forward — already in `server/sports.js` |
| Sports data never reaches Aria | `buildContext()` calls `detectNHLTeam(lower)` etc. but `lower` was never defined in that function scope — always returns `null` | Add `const lower = text.toLowerCase();` at the top of `buildContext()`, right after `const intents = detectIntent(text)` |
| Messages disappear when second question sent | Voice recognition `useEffect` runs with `[]` deps, capturing `sendMessage` from the initial render. Every voice message after the first uses the stale closure which has the original empty `messages` — wiping chat history | Add `sendMessageRef = useRef(null)`, set `sendMessageRef.current = sendMessage` on every render, then call `sendMessageRef.current(text)` inside the voice callback instead of `sendMessage(text)` directly |
